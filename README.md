...
## Дашборд: доступ к Google-таблице

Вкладка «Дашборд» читает реестр судебных дел из Google-таблицы через сервисный
аккаунт. Разовая настройка:

1. Создать проект в Google Cloud Console и включить в нём Google Sheets API.
2. Создать сервисный аккаунт и скачать его **JSON-ключ**.
3. Расшарить таблицу на адрес сервисного аккаунта с правом **«Просмотр»**.
4. В приложении: вкладка «Дашборд» → «Настройки» → указать путь к ключу и ID
   таблицы (он в ссылке: `docs.google.com/spreadsheets/d/`**`<ID>`**`/edit`).

Ключ остаётся файлом на диске пользователя: в репозиторий и в собранный `.exe`
он не попадает. Доступ запрашивается только на чтение — приложение ничего в
таблице не меняет.
...
| `load_dashboard_data` | показатели дашборда из Google-таблицы (только чтение) |
